...
#
##
function uninstall_all_cli() {
  local main_sh_path=$(get_full_path $(get_main_sh_path))
  ...
  for cli_name in ${all_cli}; do
    ...
    local profider=${DOTFILES_CLI_PATH}/${cli_name}/${cli_name_without_number}_uninstaller/${cli_name_without_number}_uninstaller_provider.sh
    import "@/${profider}"
  done
}
